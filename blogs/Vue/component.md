...
```vue
  <valid-code :value.sync="validCode" @refreshCode="getCode" ref="code"></valid-code>
```
    ...
    createdCode() {
      ...
      for (let i = 0; i < len; i++) {
        ...
        codeList.push({
          ...
          transform: `rotate(${Math.floor(Math.random() * 90) - Math.floor(Math.random() * 90)
            }deg)`,
        });
      }
          // 指向
          ...
          // 将当前数据派发出去
          this.$emit("refreshCode", codeList.map((item) => item.code).join(""));
        }
      })
